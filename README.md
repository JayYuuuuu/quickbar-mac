...
| **主图丢进 PS** | 在 Finder 里选中几个商品文件夹，旁边浮出一颗按钮，点一下每件的**首图**都在 Photoshop 里打开（去水印那道工序） |
...
- **每件只开首图**（`主图/` 里的第一张）——水印一般只烧在第一张上。全开的代价是实打实的：
  一批 63 件 = 五百多张标签页，人还得自己认哪张要改。超过 30 张会先问一声。
- 只认 `主图/`，**不带 `主图1比1/`** —— 那是另一套回填图，要不要一起改是个决定，
  不该由一个按钮偷偷替人做。真要改这两条，`MainImages` 里 `FIRST_ONLY` / `SUB_DIRS` 各一行。
