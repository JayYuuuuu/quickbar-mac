...
| **主图丢进 PS** | 在 Finder 里选中几个商品文件夹 → 菜单栏点一下，它们的「主图」全部在 Photoshop 里打开（去水印那道工序） |
...
- **菜单栏「主图丢进 PS」**：在 Finder 里选中几个商品文件夹（或直接选整个批次目录），点一下，
  它们的 `主图/` 全部在 Photoshop 里打开成标签页。一个都没选就用当前窗口那个文件夹。
  超过 30 张会先问一声（整批 = 五百多张，PS 会被打爆）。
- **网页上单件的「主图丢进 PS」**：品牌素材变动页的抽屉里，只想改这一件时用它。
- 只开 `主图/`，**不带 `主图1比1/`** —— 那是另一套回填图，要不要一起改是个决定，
  不该由一个按钮偷偷替人做。真要一起开，改 `MainImages.SUB_DIRS`。
